...
fit_gam <- gam(SalePrice ~ s(GrLivArea, by = Neighborhood)+s(LotArea)+
                 s(YearsSinceBuilt)+OQualCond+EQualCond, data = df_tr_baked)
summary(fit_gam)
sqrt(mean((df_tr_test$SalePrice-predict(fit_gam, df_tr_test))^2))
# plot(fit_gam)

ggplot(df_train, aes(x = ExterCond, y = SalePrice))+
  geom_point()+
...
fit_gam <- caret::train(rec, data = df_train,
                        method = "gam", metric = "RMSE",
                        # tuneGrid = glmGrid,

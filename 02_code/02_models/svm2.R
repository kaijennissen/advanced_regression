...
df_ts$SalePriceFit_svm2 <- predict.train(fit, newdata = df_ts) * sale_sd + sale_mean
RMSE(pred = df_ts$SalePriceFit_svm2, obs = df_ts$SalePrice)
...
df_tt$SalePriceFit_svm2 <- predict.train(fit, newdata = df_tt) * sale_sd + sale_mean
RMSE(pred = df_tt$SalePriceFit_svm2, obs = df_tt$SalePrice)

write_csv(
  x = dplyr::select(df_tt, Id, starts_with("SalePrice")),
  path = "01_data/03_predictions/01_train/svm2_prediction.csv"
)


df_test$SalePriceFit_svm2 <- predict.train(fit, newdata = df_test) * sale_sd + sale_mean

write_csv(
  x = dplyr::select(df_test, Id, starts_with("SalePrice")),
  path = "01_data/03_predictions/02_test/svm2_prediction.csv"
)
